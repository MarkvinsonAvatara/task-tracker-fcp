...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	...
	var updatedCategoryRequest model.Category
	if err := c.ShouldBindJSON(&updatedCategoryRequest); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid category data"})
		return
	}
	err2 := ct.categoryService.Update(id, updatedCategoryRequest)
	if err2 != nil {
